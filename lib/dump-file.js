...
import request from 'request'; //TODO: this is deprecated use node-fetch
...
async function getLatestDumpFile() {
  try {
    ...
    if (resultDataset.data.length) {
      const distributionMetaData = resultDataset.data[0].attributes;
      ...
      return new DumpFile(distributionMetaData, resultDistribution.data[0].relationships.subject.data);
    } else {
